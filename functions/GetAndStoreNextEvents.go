...
func GetAndStoreNextThreeEvents(userId string, app *pocketbase.PocketBase) {
	...
	for i := 0; i < len(events); i++ {
		...
		if err != nil {
			...
			newEventRecord.Set("id", userId+events[i].InstanceId)
			...
			newEventRecord.Set("resources", events[i].Resources)
			newEventRecord.Set("tags", events[i].Tags)
			...
			existingRecord.Set("recurrence", events[i].Recurrence)
