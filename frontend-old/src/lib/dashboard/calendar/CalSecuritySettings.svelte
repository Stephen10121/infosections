<script lang="ts">
    ...
    $effect(() => {
        ...
        changed = enablePasswordChanged || ( enablePassword && newPasswordChanged ) || ( enablePassword && passwordScreenMessageChanged );
    });
</script>
...
<Card.Root>
    ...
    <Card.Content class="space-y-4">
        <div class="flex items-center justify-between">
            ...
            <input {...updateCalendarForm.fields.enablePassword.as("checkbox")} class="sr-only" bind:checked={enablePasswordBindable} type="checkbox" />
            <Switch
        ...
        {#if enablePasswordBindable}
            <div class="space-y-2 pt-2">
                ...
                <Input id="password" bind:value={newPasswordBindable} type="password" placeholder="Enter password" />
            </div>
            <div class="space-y-2">
                {#if passwordScreenMessageBindable.length > 0}
                    <input {...updateCalendarForm.fields.passwordScreenMessage.as("hidden", passwordScreenMessageBindable)} />
                {/if}
