...
<div class="dark rounded-xl bg-[#1f1f1f] p-4 flex flex-col gap-2" style="border: 1px solid #333333">
	<div class="dark flex items-start justify-between">
		...
		<p class="dark text-gray-400" style="white-space: nowrap;font-size: {fontScale * 14}px;">
	...
	{#if event.description && calendarCustomizations.showDescription}
		<p class="text-gray-300 leading-relaxed" style="font-size: {fontScale * 14}px;">
			{event.description}
		</p>
	{/if}
	...
	<EventResources
		resources={event.expand.resources}
		showResourcePathname={calendarCustomizations.showResourcePathname}
		showRooms={calendarCustomizations.showRooms}
		showResources={calendarCustomizations.showResources}
		{fontScale}
	/>
				...
				<div class="text-gray-300">
					{#each event.times as time, index (`anEventTime${time.name}${event.id}`)}
						<EventTimes
							{timeZone}
							today={currentDay}
							useAMPM={calendarCustomizations.useAMPM}
							{time}
							multiDayEvent={MULTI_DAY_EVENT}
						/>{#if index + 1 < event.times.length},<br />{/if}
					{/each}
				</div>
			</div>
		</div>
	...
	{#if event.expand.tags}
		<div class="flex h-fit gap-2">
			{#each event.expand.tags as tag (`taglist${tag.tag_id}${event.id}`)}
				<div
					style="background-color: #ffffff;word-wrap:break-word;white-space: nowrap;text-overflow: ellipsis;"
					class="rounded"
				>
					<span
						class="rounded px-2 py-1 font-medium"
						style="font-size: {fontScale *
							12}px;background-color: {tag.color}cc;border: 1px solid {tag.color};color: {getContrastYIQ(
							tag.color
						)};"
					>
						{tag.name}
