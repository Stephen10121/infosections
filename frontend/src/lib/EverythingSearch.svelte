<script lang="ts">
	...
	type FilteredTermsList =
		| { type: "calendar"; data: CalendarDBModel }
		| { type: "image-feed"; data: ImageFeedDBModel }
		| { type: "event-list"; data: EventListDBModel };
	...
	function searchInputChanged(event: Event) {
		...
		for (let i = 0; i < imageFeeds.length; i++) {
			...
			if (
				feed.id.toLowerCase().includes(searchTerm) ||
		...
		for (let i = 0; i < eventLists.length; i++) {
			...
			if (
				list.id.toLowerCase().includes(searchTerm) ||
	...
	function keyDown(event: KeyboardEvent) {
		...
		if (event.key === "Enter" && arrowDownIndex !== -1) {
			...
			if (selectedTerm) {
				goto(
					`/dashboard/${selectedTerm.type === "calendar" ? "calendars" : selectedTerm.type === "event-list" ? "event-lists" : "image-feeds"}/${selectedTerm.data.id}`
				);
			}
			resetSearch();
		}
...
<div class="relative w-full max-w-md">
	...
	{#if everythingInputFocused && filteredTerms.length > 0}
		<div
			class="border shadow-lg bg-card absolute left-0 -bottom-1 translate-y-full w-full max-w-md rounded-lg p-1 z-50 isolate everythingSearcher"
		>
			{#each filteredTerms as term, index (`filteredSearchTerms${term.data.id}`)}
				<a
					onclick={resetSearch}
					href="/dashboard/{term.type === 'calendar'
						? 'calendars'
						: term.type === 'event-list'
							? 'event-lists'
							: 'image-feeds'}/{term.data.id}"
					class="w-full px-4 py-3 hover:bg-accent/50 transition-colors text-left flex flex-col gap-1 rounded-md {arrowDownIndex -
						1 ===
					index
						? 'bg-accent/70'
						: ''}"
