	...
	>
		...
		>
			...
			<div class="space-y-2">
				...
				{/if}
				<div
					class="{uploadNewEventPictureLink
						? 'sr-only'
				...
				>
					<Button variant="outline" class="relative bg-transparent" type="button">
						...
						Upload Image
						<input
							{...(createCustomImageForm.fields.picture as unknown as RemoteFormField<File>).as(
								"file"
							)}
							type="file"
							accept="image/*"
			...
			<div class="flex items-center justify-between">
				<input
					{...createCustomImageForm.fields.showLink.as("checkbox")}
					checked={showLink}
					type="checkbox"
					class="sr-only"
				/>
				<div class="space-y-0.5">
					<Label for="event-show" class="text-base">Add Link</Label>
					<p class="text-sm text-muted-foreground">
						This adds a button that lets people click a custom link.
					</p>
				</div>
				<Switch id="event-show" bind:checked={showLink} />
			</div>
			{#if showLink}
				<div class="space-y-2">
					<input
						{...createCustomImageForm.fields.linkText.as("text")}
						value={linkText}
						class="sr-only"
					/>
					<Label for="event-name">Link Button Text</Label>
					<Input id="event-name" placeholder="Link Button Text" bind:value={linkText} />
				</div>
				<div class="space-y-2">
					<input
						{...createCustomImageForm.fields.registrationURL.as("url")}
