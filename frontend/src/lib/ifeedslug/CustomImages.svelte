...
{#if customImages.length > 0}
				...
				<Table.Row onclick={() => eventClicked(index)}>
					<Table.Cell>
						<img
							...
							src="{apiServer}api/files/{customEvent.collectionId}/{customEvent.id}/{customEvent.picture}"
							alt="@leerob"
						/>
					</Table.Cell>
	...
	<div class="w-full flex items-center justify-center mt-2">
		<Button variant="outline" class="w-full" onclick={() => (createCustomEventDialog = true)}
			>Add Image</Button
		>
	</div>
{:else}
...
<Sheet.Root bind:open={showEvent}>
	<Sheet.Content side="left">
		<Sheet.Header>
			...
			<Sheet.Description
				>Make changes to the image settings here. Click save when you're done.</Sheet.Description
			>
		</Sheet.Header>
		{#if showEvent}
			<UpdateCustomEvent
		...
		<Sheet.Footer>
			...
			<Dialog.Root>
				<Dialog.Trigger class={buttonVariants({ variant: "outline" })}>Delete Image</Dialog.Trigger>
				<Dialog.Content>
					<Dialog.Header>
						...
						<Dialog.Footer>
							<Button variant="destructive" type="reset" onclick={deleteEvent}
