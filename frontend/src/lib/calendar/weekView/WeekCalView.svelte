    ...
    <div class="overflow-hidden h-screen">
        <div class="grid grid-cols-7 dark bg-foreground">
            {#each days as day (`aweekday${day.dayOfWeek}`)}
                <div class="text-center py-2 text-muted-foreground dark font-semibold border-b border-[#333333]" style="font-size: {fontScale * 12}px;">
        ...
        <div class="grid grid-cols-7 h-full">
            {#each days as day, index (`adayinmonth${index}`)}
                <WeekCalViewDay {index} {events} {day} {currentDate} {timeZone} {calendarCustomizations} {fontScale} />
            {/each}
        </div>
    </div>
</div>