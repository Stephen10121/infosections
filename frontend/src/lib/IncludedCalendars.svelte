<script lang="ts">
	import {
		removeIncludedCalendarsToIFeed,
		updateIncludedCalendarsToIFeed
	} from "../routes/(mainWebsite)/dashboard/image-feeds/IFeedIncludeCalActions.remote";
	...
	async function includeCalendar() {
		...
		const updatingIncludedCalendars = toast.loading("Updating List", {
	...
	async function removeCal(calId: string) {
		const removingCalToast = toast.loading("Removing included calendar", {
			duration: Number.POSITIVE_INFINITY
		});
	...
	{#if includedCalendars.length > 0}
		{#each includedCalendars as calendar (`includedcals${calendar}`)}
			<AnIncludedCalendar
				...
				calendarId={calendar}
				{pb_url}
				removeAdditionalCalendar={removeCal}
			/>
		{/each}
		<div class="w-full flex items-center justify-center mt-2">
			<Button variant="outline" class="w-full" onclick={() => (includeACalendarPopup = true)}
...
<Dialog.Root bind:open={includeACalendarPopup}>
	<Dialog.Content
		class="sm:max-w-125 max-h-screen overflow-y-auto"
			...
			{#if includedCalendars.length === calendars.length}
				<Empty.Root>
					<Empty.Header>
						...
						<Empty.Title
							>No {#if calendars.length !== 0}Extra{/if} Calendars</Empty.Title
						>
						<Empty.Description>
