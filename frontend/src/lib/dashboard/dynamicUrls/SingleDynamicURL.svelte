<script lang="ts">
	...
	let {
		...
		url,
		websiteURL
	}: { openId: string | null; url: DynamicURLModel; websiteURL: string } = $props();

	function getStatusInfo(url: DynamicURLModel) {
		if (url.disableURL)
			return { label: "Disabled", variant: "destructive" as const, icon: ShieldOff };
		if (url.enableOverrideRedirect)
			return { label: "Override Active", variant: "secondary" as const, icon: ShieldAlert };
		if (url.enableWeekSheet)
			return { label: "Scheduled", variant: "default" as const, icon: Globe };
	...
	>
		...
		<Collapsible.Content>
			<div class="border-t border-border">
				<DynamicURLEditor {url} bind:saveRequired />
			</div>
		</Collapsible.Content>
	</Card.Root>
</Collapsible.Root>
