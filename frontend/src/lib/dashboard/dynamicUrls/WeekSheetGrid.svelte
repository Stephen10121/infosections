<!-- <script lang="ts">
    ...
    let { weekSheet, onchange }: { weekSheet: WeekSheetTimeSlot[][], onchange: (newWeekSheet: WeekSheetTimeSlot[][]) => unknown } = $props();
...
<script lang="ts">
  ...
  function handleUpdateSlotLink(dayIndex: number, slotIndex: number, link: string) {
    const updated = [...weekSheet];
    updated[dayIndex] = updated[dayIndex].map((s, i) =>
      i === slotIndex ? { ...s, link } : s
    );
    onchange(updated);
  }
      ...
      <div class="rounded-lg border border-border overflow-hidden bg-card">
        <div class="grid border-b border-border" style="grid-template-columns: 56px repeat(7, 1fr);">
          ...
          {#each DAY_LABELS as day (`aDay${day}`)}
            <div
              class="p-2 text-xs font-semibold text-center text-foreground border-l border-border"
            >
              {day}
            </div>
          {/each}
