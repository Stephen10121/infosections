<script lang="ts">
	import {
		deleteDynamicURLCommand,
		updateDynamicURLCommand
	} from "../../../routes/(mainWebsite)/dashboard/dynamic-urls/data.remote";
	...
	let { url, saveRequired = $bindable() }: { url: DynamicURLModel; saveRequired: boolean } =
		$props();
	...
	let selectedOverrideExpireTime: (typeof overrideExpiresTimes)[number]["value"] | "set" = $derived(
		url.overrideExpireInStr
	);
	...
	async function update() {
		...
		try {
			const response = await updateDynamicURLCommand({
				...
				disableURL,
				overrideExpiresIn:
					selectedOverrideExpireTime === "never"
						? ""
						: selectedOverrideExpireTime === "set"
							? url.overrideExpiresIn
							: Temporal.Now.zonedDateTimeISO(currentTz)
									.add({ minutes: selectedOverrideExpireTimeToDate })
...
<div class="p-4 lg:p-6 space-y-8 relative">
	...
	<section class="space-y-4">
		...
		<div class="flex items-center justify-between rounded-lg border border-border p-4">
			<div class="space-y-0.5">
				<Label for={`disable-${url.id}`} class="font-medium">Disable URL</Label>
				<p class="text-xs text-muted-foreground">
	...
	<section class="space-y-4">
		...
		<div class="rounded-lg border border-border p-4 space-y-4">
			<div class="flex items-center justify-between">
				<div class="space-y-0.5">
					<Label for={`override-toggle-${url.id}`} class="font-medium">Enable Override</Label>
					<p class="text-xs text-muted-foreground">
					...
					<Label class="flex flex-col items-start mt-5">
						...
						<div class="flex items-center gap-2">
							<Select.Root
								type="single"
								name="favoriteFruit"
								bind:value={selectedOverrideExpireTime}
							>
								...
								<Select.Content>
									<Select.Group>
										{#each overrideExpiresTimes as expireTime (expireTime.value)}
											<Select.Item value={expireTime.value} label={expireTime.label}>
												{expireTime.label}
											</Select.Item>
										{/each}
									</Select.Group>
	...
	<section class="space-y-4">
		...
		<div class="rounded-lg border border-border p-4 space-y-4">
			<div class="flex items-center justify-between">
				<div class="space-y-0.5">
					<Label for={`week-toggle-${url.id}`} class="font-medium">Enable Weekly Schedule</Label>
					<p class="text-xs text-muted-foreground">
