	...
	<div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
		...
		>
			...
			<Button
				variant="outline"
				size="sm"
				class="w-full mt-4 border-border/50 hover:bg-secondary"
			...
			<div class="space-y-1">
				{#each myEventLists as list (`alist${list.id}`)}
					<ViewItem
						...
						{timeZone}
						logo={list.logo
							? `${pb_url}/api/files/${list.collectionId}/${list.id}/${list.logo}`
							: null}
						name={list.name}
						visits={list.visits}
			...
			<div class="space-y-1">
				{#each myImageFeeds as feed (`afeed${feed.id}`)}
					<ViewItem
						...
						{timeZone}
						logo={feed.logo
							? `${pb_url}/api/files/${feed.collectionId}/${feed.id}/${feed.logo}`
							: null}
						name={feed.name}
						visits={feed.visits}
		...
		>
			<div class="grid gap-0 sm:grid-cols-3">
				{#each myDynamicURLs as url (`adurl${url.id}`)}
					<DynamicUrlItem name={url.id} redirect={307} hits={100} />
				{/each}
			</div>
			<Button
				variant="outline"
