<script lang="ts">
    ...
    let todaysEvents = $derived((await getAllUserEvents()).filter((event) => {
        ...
        {#if todaysEvents.length > 0}
            <div class="space-y-3">
                {#each todaysEvents as event (`atodayevent${event.id}`)}
                    <div class="flex items-start gap-4 p-3 rounded-lg bg-secondary/50 hover:bg-secondary transition-colors">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-muted-foreground flex items-center gap-1 mt-2">
                                ...
                                <Time date={Temporal.Instant.from(event.endTime).toZonedDateTimeISO(timeZone)} useAMPM />
                            </p>
                        </div>
                    </div>
                {/each}
