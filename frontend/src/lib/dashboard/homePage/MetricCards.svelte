<script lang="ts">
	import { cn, type DynamicURLModel, type EventListDBModel, type ImageFeedDBModel } from "@/utils";
	...
	const totalVisits = $derived(
		myCalendars.map((cal) => cal.visits).reduce((a, b) => a + b, 0) +
			...
			myDynamicURLs
				.map((dURL) => dURL.refs.map((aref) => aref.hits).reduce((a, b) => a + b, 0))
				.reduce((a, b) => a + b, 0)
	);
...
<div class="grid gap-4 grid-cols-2 md:grid-cols-3 lg:grid-cols-6">
	{#each metricConfig as config (`ametriccard${config.key}`)}
		<Card.Root
			class="relative overflow-hidden border-border/50 bg-card hover:bg-card/80 transition-colors"
		>
