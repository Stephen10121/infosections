<script lang="ts">
	...
	let nextSync = $derived(
		timeWhen(new Date(mostRecentSync).setHours(mostRecentSync.getHours() + 1))
	);
						...
						<div class="flex items-center gap-2.5">
							...
							<div>
								...
								<p class="text-xs text-muted-foreground">
									{allUserEvents.filter((event) => event.service === integration.service).length} events
									&middot; {timeAgo(new Date(integration.lastEventsFetch))}
								</p>
							</div>
						</div>

						<div
							class="text-xs px-2 py-0.5 rounded-full {isConnected
