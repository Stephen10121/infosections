<script lang="ts">
    ...
    const recentActivity = $derived([
        ...myCalendars.map((cal) => {return {...cal, type: "calendars"}}) as (CalendarDBModel & ActivityType)[],
        ...myImageFeeds.map((feed) => {return {...feed, type: "image-feeds"}}) as (ImageFeedDBModel & ActivityType)[],
        ...myEventLists.map((list) => {return {...list, type: "event-lists"}}) as (EventListDBModel & ActivityType)[],
        ...myDynamicURLs.map((url) => {return {...url, type: "dynamic-urls"}}) as (DynamicURLModel & ActivityType)[]
    ].sort((a, b) => Date.parse(b.updated) - Date.parse(a.updated)).slice(0, 10));
            ...
            {#each recentActivity as activity (`aRecentActivity${activity.id}`)}
                ...
                <div class="flex items-center justify-between py-2.5 border-b border-border/50 last:border-0">
                    ...
                    <div class="flex items-center gap-1.5 text-sm text-muted-foreground shrink-0">
                        <Button variant="ghost" size="sm" class="h-8 text-xs" href="/dashboard/{activity.type}{config.naming !== "id" ? `/${activity.id}` : `?expanded=${activity.id}`}" title="View Details">
