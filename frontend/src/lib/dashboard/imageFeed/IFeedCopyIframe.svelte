...
<Dialog.Root bind:open={iframeDialogOpen}>
    ...
    <Dialog.Content class="sm:max-w-lg max-h-screen overflow-y-auto">
        ...
        <div class="space-y-2">
            ...
            <div class="relative">
                <pre class="p-3 rounded-md bg-muted text-xs font-mono overflow-x-auto max-h-80 whitespace-pre-wrap break-all">{generateIframeCode(`${infosectionsLink}/ifeed/${feedId}`, `Infosections Feed | ${feedName}`, feedId, infosectionsLink, true)}</pre>
            </div>
        </div>
