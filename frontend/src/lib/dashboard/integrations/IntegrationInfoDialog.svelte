<script lang="ts">
	...
	async function handleSync() {
		if (integration?.service === "planningcenter") {
			...
			const myIntegrations = (await getMyIntegrations()).filter(
				(int) => int.service === "planningcenter"
			);
			integration =
				myIntegrations.length > 0 ? (myIntegrations[0] ? myIntegrations[0] : null) : null;
		}
	}
			...
			<Dialog.Header>
				<div class="flex items-center gap-3">
					...
					<div>
						...
						<Dialog.Description class="mt-0.5"
							>Manage your {integration.prettyName} integration</Dialog.Description
						>
					</div>
				</div>
			</Dialog.Header>
			...
			<Dialog.Footer class="flex-col gap-2 sm:flex-col">
				{#if integration.service === "planningcenter"}
					<div class="flex gap-2 w-full">
						<Button variant="outline" class="flex-1" onclick={handleSync} disabled={syncing}>
							{#if syncing}
				...
				{/if}
				<form
					{...removeAnIntegration.enhance(async ({ submit }) => {
						let loading = toast.loading("Removing Integration.", {
