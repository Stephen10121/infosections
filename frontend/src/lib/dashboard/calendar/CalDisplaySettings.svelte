		...
		<div class="grid grid-cols-1 gap-6">
			<div class="flex items-center justify-between flex-col space-y-2">
				<Label for="showDescription" class="flex flex-col items-start space-y-1 cursor-pointer">
					...
					<span class="text-sm text-muted-foreground"
						>You can choose a 3 day, week, or a month view option. The smaller the view, more
						information can be displayed.</span
					>
				</Label>
				<input
			...
			<div class="flex items-center justify-between space-x-2">
				...
				<input
					...
					type="checkbox"
				/>
				<Switch id="showDescription" bind:checked={displaySettingsBindable.showDescription} />
			</div>
			...
			<div class="flex items-center justify-between space-x-2">
				...
				<input
					...
					type="checkbox"
				/>
				<Switch id="useAMPM" bind:checked={displaySettingsBindable.useAMPM} />
			</div>
			...
			<div class="flex items-center justify-between space-x-2">
				<Label for="showResources" class="flex flex-col items-start space-y-1 cursor-pointer">
					...
					<span class="text-sm text-muted-foreground"
						>Display the resources that the event needs.</span
					>
				</Label>
				<input
					...
					type="checkbox"
				/>
				<Switch id="showResources" bind:checked={displaySettingsBindable.showResources} />
			</div>

			{#if displaySettingsBindable.showResources}
				<div class="flex items-center justify-between space-x-2 ml-5">
					<Label
						for="showResourcePathname"
			...
			<div class="flex items-center justify-between space-x-2">
				...
				<input
					...
					type="checkbox"
				/>
				<Switch id="showRooms" bind:checked={displaySettingsBindable.showRooms} />
			</div>
			...
			<div class="flex items-center justify-between space-x-2">
				...
				<input
					...
					type="checkbox"
				/>
				<Switch id="showLocation" bind:checked={displaySettingsBindable.showLocation} />
			</div>

			{#if displaySettingsBindable.showLocation}
				<div class="flex items-center justify-between space-x-2 ml-5">
					<Label
						for="onlyShowLocationTitle"
						class="flex flex-col items-start space-y-1 cursor-pointer"
					>
