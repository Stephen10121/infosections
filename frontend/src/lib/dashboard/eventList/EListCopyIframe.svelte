<script lang="ts">
	...
	function copyIframeCode() {
		const link = generateIframeCode(
			`${infosectionsLink}/elist/${listId}`,
			`Infosections List | ${listName}`,
			listId,
			infosectionsLink
		);
		...
		<div class="flex gap-3 p-3 rounded-md bg-amber-500/10 border border-amber-500/20">
			...
			<div class="text-sm">
				...
				<p class="text-muted-foreground mt-1">
					The script tag is required because it automatically adjusts the height of the iframe when
					content inside changes. If you dont want to include the script tag. Enable scrolling in
					the iframe to ensure users can see everything.
				</p>
			</div>
		</div>
