<script lang="ts">
    ...
    let { events, day, dayNumber, calendarCustomizations, timeZone, filters, fontScale }: { events: EventDBModelExpanded[], day: Temporal.ZonedDateTime, dayNumber: number, calendarCustomizations: CalendarCustomizations, timeZone: Temporal.TimeZoneLike, filters: CalendarFilters, fontScale: number } = $props();
...
<div class="dark flex flex-col gap-4">
    <div class="dark flex flex-col gap-1">
        ...
        <div class="dark font-semibold text-accent" style="font-size: {fontScale * 20}px;">{LONGDAYTOSTRING[day.dayOfWeek]}, {MONTHTOSTRING[day.month]} {day.day}</div>
    </div>
