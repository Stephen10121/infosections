<script lang="ts">
	...
	afterNavigate(() => {
		if (browser) {
			newIFeedDialogOpen = $page.url.searchParams.get("new") === "1";
		}
	});
</script>
...
<div class="w-full h-full space-y-6">
		...
		<Dialog.Root bind:open={newIFeedDialogOpen}>
			...
			<Dialog.Content class="sm:max-w-125">
				<Dialog.Header>
					...
					<Dialog.Description
						>You can change more settings after creating the feed.</Dialog.Description
					>
				</Dialog.Header>
				...
				<div class="space-y-4 py-4">
					<div class="space-y-2">
						...
						<Input id="name" placeholder="e.g., Church Website Feed" bind:value={newIFeedName} />
					</div>
				...
				<Dialog.Footer>
					<Button variant="outline" onclick={() => (newIFeedDialogOpen = false)}>Cancel</Button>
	...
	<div class="w-full h-full grid gap-4 xl:grid-cols-3 relative">
		{#if (await getMyImageFeeds()).length === 0}
			<p class="absolute top-3 left-1/2 -translate-1/2 text-muted-foreground">
				No Image Feeds Yet.
			</p>
		{/if}
					...
					<div class="flex items-start justify-between">
						<div class="flex items-center gap-1">
							<Avatar.Root class="h-15 w-15">
								<Avatar.Image
									src="{data.pb_url}/api/files/{imageFeed.collectionId}/{imageFeed.id}/{imageFeed.logo}"
									alt="Avatar"
								/>
						...
						<DropdownMenu.Root>
							...
							<DropdownMenu.Content align="end">
								<DropdownMenu.Item onclick={() => copyIFeedLinkToClipboard(imageFeed.id)}>
