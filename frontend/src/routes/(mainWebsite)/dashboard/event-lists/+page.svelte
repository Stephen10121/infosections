		...
		<Dialog.Root bind:open={newEListDialogOpen}>
			...
			<Dialog.Content class="sm:max-w-125">
				<Dialog.Header>
					...
					<Dialog.Description
						>You can change more settings after creating the list.</Dialog.Description
					>
				</Dialog.Header>
				...
				<div class="space-y-4 py-4">
					<div class="space-y-2">
						...
						<Input id="name" placeholder="e.g., Church Website List" bind:value={newEListName} />
					</div>
				...
				<Dialog.Footer>
					<Button variant="outline" onclick={() => (newEListDialogOpen = false)}>Cancel</Button>
					...
					<div class="flex items-start justify-between">
						<div class="flex items-center gap-1">
							<Avatar.Root class="h-15 w-15">
								<Avatar.Image
									src="{data.pb_url}/api/files/{eventList.collectionId}/{eventList.id}/{eventList.logo}"
									alt="Avatar"
								/>
						...
						<DropdownMenu.Root>
							...
							<DropdownMenu.Content align="end">
								<DropdownMenu.Item onclick={() => copyEListLinkToClipboard(eventList.id)}>
