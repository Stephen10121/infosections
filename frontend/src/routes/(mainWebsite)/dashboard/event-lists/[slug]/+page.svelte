            ...
            if (saveChangesToast === null) {
                saveChangesToast = toast("Save?", {
                    ...
                    action: {
                        label: "Save Changes",
                        onClick: () => {
                            const formSubmitButton = document.getElementById("updateEventListButton1") as HTMLButtonElement | null;
                            if (formSubmitButton) {
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            >
                ...
                <EListAvatar
                    ...
                    avatarLink={selectedEventList.logo ? `${data.pb_url}/api/files/${selectedEventList.collectionId}/${selectedEventList.id}/${selectedEventList.logo}` : ""}
                />
                ...
                <EListDisplaySettings displaySettings={selectedEventList.displaySettings} bind:changed={displaySettingsChanged} bind:displaySettingsPreview />

                <EListPreview displaySettings={displaySettingsPreview} listId={selectedEventList.id} />

                <EListFilterSettings filters={selectedEventList.filters} bind:changed={filterSettingsChanged} />
            </form>
        </div>
