...
<div class="space-y-6">
	...
	<div class="space-y-3">
		{#each await getMyDynamicURLS() as dynamic_url (`aDynamicURL${dynamic_url.id}`)}
			<SingleDynamicURL bind:openId url={dynamic_url} websiteURL={data.websiteURL} />
		{/each}
	...
	<Dialog.Content class="sm:max-w-125">
		...
		<div class="space-y-4 py-4">
			<div class="space-y-2">
				...
				<Input id="id" placeholder="e.g., donate" bind:value={newDynamicURLID} />
			</div>
		...
		<Dialog.Footer>
			<Button variant="outline" onclick={() => (newDynamicURLOpen = false)}>Cancel</Button>
