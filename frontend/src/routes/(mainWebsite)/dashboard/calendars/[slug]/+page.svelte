<script lang="ts">
    ...
    $effect(() => {
        const saveRequired = avatarChanged || generalInfoChanged || displaySettingsChanged || filterSettingsChanged || securitySettingsChanged;
            ...
            if (saveChangesToast === null) {
                saveChangesToast = toast("Save?", {
                    ...
                    action: {
                        label: "Save Changes",
                        onClick: () => {
                            const formSubmitButton = document.getElementById("updateCalendarButton1") as HTMLButtonElement | null;
                            if (formSubmitButton) {
...
<div class="max-w-5xl mx-auto space-y-6 isolate">
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        <form {...updateCalendarForm.enhance(async ({ submit, form }) => {
            ...
            <CalAvatar
                ...
                avatarLink={selectedCalendar.logo ? `${data.pb_url}/api/files/${selectedCalendar.collectionId}/${selectedCalendar.id}/${selectedCalendar.logo}` : ""}
            />
            ...
            <CalDisplaySettings displaySettings={selectedCalendar.displaySettings} bind:changed={displaySettingsChanged} bind:displaySettingsPreview />
