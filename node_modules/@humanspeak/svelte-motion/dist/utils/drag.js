...
import { startDragInertia } from './dragInertia';
...
import { buildGestureTransform, collectGestureTransformValues as collectTransformValues, splitGestureTransformValues as splitTransformValues } from './transformComposer';
import { animate } from 'motion';
import { animateMotionValue, animateValue, getValueAsType, motionValue, numberValueTypes } from 'motion-dom';
/**
...
const mix = (min, max, progress) => min + (max - min) * progress;
const defaultElastic = 0.35;
const clampElastic = (value) => Math.max(0, Math.min(1, value));
const resolvePointElastic = (elastic, key) => (typeof elastic === 'number' ? elastic : (elastic[key] ?? 0));
const resolveAxisElastic = (elastic, minKey, maxKey) => ({
    min: clampElastic(resolvePointElastic(elastic, minKey)),
    max: clampElastic(resolvePointElastic(elastic, maxKey))
});
const resolveDragElastic = (elastic = defaultElastic) => {
    if (elastic === false)
        elastic = 0;
    if (elastic === true)
...
const getMaxElastic = (elastic) => Math.max(elastic.x.min, elastic.x.max, elastic.y.min, elastic.y.max);
const calcConstraintProgress = (value, min, max) => {
 ...
 * ```
 */
const getElasticFactor = (elastic, side) => {
    if (elastic == null)
        return 0;
 ...
 * ```
 */
const applyDragOriginConstraints = (value, dragOrigin, range, elastic) => {
 ...
 */
export const resolveConstraints = (el, constraints) => {
    if (!constraints)
        return null;
 ...
 */
export const applyElastic = (value, min, max, elastic) => {
    if (value < min)
        return min + (value - min) * Math.max(0, Math.min(1, elastic));
    if (value > max)
        return max + (value - max) * Math.max(0, Math.min(1, elastic));
    return value;
};
 ...
 */
const sessionListenerOptions = { passive: true, capture: true };
const addPixelOffset = (value, offset) => {
    ...
    if (pxValue)
        return `${Number.parseFloat(pxValue[1]) + offset}px`;
...
export const attachDrag = (el, opts) => {
    ...
    const markDragTransformActive = (active) => {
        if (active) {
    ...
    const getConstraintBounds = (base, currentConstraints) => ({
    ...
    const scalePositionWithinConstraints = () => {
        ...
        const nextY = applyY
            ...
            : applied.y;
        if (nextX === applied.x && nextY === applied.y)
            return;
    ...
    const stopConstraintResizeObserver = isDomElement(opts.constraints) && typeof ResizeObserver !== 'undefined'
        ? (() => {
    ...
    const setXYImmediate = (x, y) => {
        ...
        if (boundX && boundX.get() !== x)
            boundX.set(x);
        if (boundY && boundY.get() !== y)
            boundY.set(y);
        const latestValues = {
        ...
        if (dragX && !boundX)
            latestValues.x = addPixelOffset(latestValues.x, x);
        if (dragY && !boundY)
            latestValues.y = addPixelOffset(latestValues.y, y);
        ...
        const shouldWrite = (dragX && !boundX) ||
            (dragY && !boundY) ||
            crossAxisOffset.x !== 0 ||
            crossAxisOffset.y !== 0 ||
        ...
        if (shouldWrite) {
            composedTransform =
                buildDragTransform(latestValues, opts.getBaseTransform?.() ?? '', opts.transformTemplate) || 'none';
            el.dataset.svelteMotionDragTransform = composedTransform;
            const writeComposedTransform = () => {
                if (el.dataset.svelteMotionDragTransform !== composedTransform)
                    return;
    ...
    const stopTransformAnimation = (key) => {
    ...
    const stopTransformAnimations = () => {
        for (const key of [...transformAnimations.keys()])
            stopTransformAnimation(key);
    };
    const normalizeTransformTarget = (key, target) => {
        const normalize = (value) => {
            if (value === null)
                return null;
            if (typeof value !== 'string' && typeof value !== 'number')
                return undefined;
            const typedValue = getValueAsType(value, numberValueTypes[key]);
            return typeof typedValue === 'string' || typeof typedValue === 'number'
                ? typedValue
                : value;
        };
        if (!Array.isArray(target))
            return normalize(target) ?? undefined;
        const keyframes = target.map(normalize).filter((value) => value !== undefined);
        return keyframes.length ? keyframes : undefined;
    };
    const startTransformAnimation = (key, target, transition) => {
        ...
        const current = getValueAsType(gestureTransformValues[key] ??
            restingTransformValues[key] ??
            baseValues[key] ??
            neutral, numberValueTypes[key]);
        ...
        setXYImmediate(applied.x, applied.y);
        void value.start(animateMotionValue(key, value, normalizedTarget, (transition ?? mergedTransition)));
    ...
    const adjustOrigin = (dx, dy) => {
        ...
        if (!dragX)
            crossAxisOffset.x += dx;
        if (!dragY)
            crossAxisOffset.y += dy;
    ...
    const startWhileDrag = () => {
        ...
        whileDragBaseline = computeHoverBaseline(el, {
            ...
            whileHover: (opts.whileDrag ?? {}),
            baseValues: opts.getBaseTransformValues?.()
        });
        const { keyframes, transition } = splitHoverDefinition(opts.whileDrag);
        ...
        if (Object.keys(nativeKeyframes).length > 0) {
            animate(el, nativeKeyframes, transition ?? mergedTransition);
        }
    };
    const endWhileDrag = () => {
        ...
        const restore = Object.keys(nativeBaseline).length > 0
            ? animate(el, nativeBaseline, mergedTransition)
            : null;
        whileDragBaseline = null;
        const finishRestore = () => {
            if (generation !== whileDragAnimationGeneration)
                return;
            stopTransformAnimations();
            for (const key of Object.keys(transform))
                delete gestureTransformValues[key];
            setXYImmediate(applied.x, applied.y);
            whileDragRestoreActive = false;
            maybeStopTransformComposer();
        };
        ...
        for (const [key, target] of Object.entries(transform)) {
            ...
            if (control)
                restorePromises.push(control.finished);
        }
     ...
     */
    const beginDrag = (e, snapToCursor = false) => {
        pwLog('[drag] begin', {
            el: EL_ID,
        ...
        if (snapToCursor) {
            ...
            if (applyXAxis)
                applied.x = rendered.tx - base.tx;
            if (applyYAxis)
                applied.y = rendered.ty - base.ty;
            const rect = el.getBoundingClientRect();
            const centerX = rect.left + rect.width / 2;
            const centerY = rect.top + rect.height / 2;
            if (applyXAxis)
                origin.x = applied.x + e.clientX - centerX;
            if (applyYAxis)
                origin.y = applied.y + e.clientY - centerY;
        ...
        window.addEventListener('pointermove', onPointerMove, sessionListenerOptions);
        window.addEventListener('pointerup', onPointerUp, sessionListenerOptions);
        window.addEventListener('pointercancel', onPointerCancel, sessionListenerOptions);
    };
    /**
    ...
    const finishDrag = (e, cancelled = false) => {
        ...
        window.removeEventListener('pointermove', onPointerMove, sessionListenerOptions);
        window.removeEventListener('pointerup', onPointerUp, sessionListenerOptions);
        window.removeEventListener('pointercancel', onPointerCancel, sessionListenerOptions);
        ...
        if (momentum && !cancelled) {
            ...
            const { power, timeConstant, restDelta, restSpeed, bounceStiffness, bounceDamping } = deriveBoundaryPhysics(maxElastic, opts.transition);
            ...
            if (applyX) {
                animations.push(startDragInertia({
                    value: applied.x,
                    velocity: velocity.x,
                    min: inertiaMinX,
                    max: inertiaMaxX,
            ...
            let running = true;
            const animations = [];
            const { timeConstant, restDelta, restSpeed, bounceStiffness, bounceDamping } = deriveBoundaryPhysics(maxElastic, opts.transition);
            const renderLatest = () => {
                if (!running)
                    return;
                setXYImmediate(latestX, latestY);
            ...
            const finishSettle = () => {
                if (!running)
                    return;
                if (applyX)
                    applied.x = applyFloatConstraints(latestX, { min: minX, max: maxX });
                if (applyY)
                    applied.y = applyFloatConstraints(latestY, { min: minY, max: maxY });
                setXYImmediate(applied.x, applied.y);
                running = false;
                stopInertia = null;
                postReleaseAnimationActive = false;
            ...
            const addSettleAnimation = (from, to, min, max, onUpdate) => {
                ...
                remainingAnimations++;
                animations.push(shouldSpringBoundary
                    ? startDragInertia({
                        value: from,
                        velocity: 0,
                        min,
