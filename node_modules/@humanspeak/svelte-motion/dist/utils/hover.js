import { buildGestureTransform, collectGestureTransformValues } from './transformComposer';
 ...
 */
export const isHoverCapable = (win = window) => {
    try {
 ...
 */
export const splitHoverDefinition = (def) => {
    const { transition, ...rest } = (def ?? {});
    return { keyframes: rest, transition };
};
const readTransformScale = (el) => {
...
const getFinalNumber = (value) => {
    const raw = Array.isArray(value) ? value[value.length - 1] : value;
    const parsed = typeof raw === 'number' ? raw : Number.parseFloat(String(raw));
    return Number.isFinite(parsed) ? parsed : null;
};
const toMillisecondsTransition = (transition) => {
    const valueTransition = { ...(transition ?? {}) };
 ...
 */
export const computeHoverBaseline = (el, opts) => {
    const baseline = {};
 ...
 */
export const attachWhileHover = (el, whileHover, mergedTransition, callbacks, baselineSources, transformComposer) => {
    if (!whileHover)
        return () => { };
    ...
    const writeComposedScale = (scale) => {
        const transform = buildGestureTransform({
            ...(transformComposer?.getBaseTransformValues?.() ?? {}),
            ...restingTransformValues,
            ...(transformComposer?.getLiveTransformValues?.() ?? {}),
            scale
        }, transformComposer?.getBaseTransform?.() ?? fallbackBaseTransform, transformComposer?.transformTemplate) || 'none';
    ...
    const animateScale = (target, transition, onComplete) => {
    ...
    const cleanupHover = hover(el, () => {
        if (el.dataset.svelteMotionDragActive === 'true')
            return () => { };
        ...
        return () => {
            ...
            if (hoverBaseline && Object.keys(hoverBaseline).length > 0) {
                ...
                if (baselineScale != null)
                    animateScale(baselineScale, mergedTransition);
