...
const captureScrollSnapshot = (element) => {
    const snapshots = [];
...
const measureScrollDelta = (snapshots) => {
    return snapshots.reduce((delta, snapshot) => {
...
const translateRectByScrollDelta = (rect, delta) => {
...
export const createAnimatePresenceContext = (context) => {
    ...
    const exitPlaceholders = new Map();
    /**
    ...
    const refreshSiblingAnchors = () => {
        ...
        const ownPlaceholders = new Set(exitPlaceholders.values());
        for (const child of children.values()) {
     ...
     * ```
     */
    const resolvePlaceholderAnchor = (child) => {
        if (child.element.isConnected)
    ...
    let inFlightExits = 0;
    const removeExitPlaceholder = (key, placeholder) => {
    ...
    const unregisterChild = (key) => {
        ...
        const insertionParent = (child.element.parentElement?.isConnected ? child.element.parentElement : null) ??
